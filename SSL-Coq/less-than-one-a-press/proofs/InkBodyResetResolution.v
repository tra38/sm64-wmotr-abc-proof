(** Resolve the actual body-reset helper in both selected linked programs. *)
...
From LessThanOneAPress.Proofs Require Import GameTypes InkBodyResetFrame
...
Lemma ibr_us_reset_source :
  nth_error us_mario.global_definitions
    (ueqr_definition_index us_mario._mario_reset_bodystate us_mario.global_definitions) =
  Some (us_mario._mario_reset_bodystate, Gfun (Internal (ibr_body VersionUS))).
Proof. vm_compute; reflexivity. Qed.

Lemma ibr_us_reset_member :
  In (us_mario._mario_reset_bodystate, Gfun (Internal (ibr_body VersionUS)))
...
Proof.
  ...
  - eapply nth_error_In. exact ibr_us_reset_source.
Qed.

Lemma ibr_us_reset_selection :
  PTree.get us_mario._mario_reset_bodystate us_normalized_global_definition_map =
    Some (Gfun (Internal (ibr_body VersionUS))).
Proof.
  ...
  - exact ibr_us_reset_member.
Qed.

Lemma ibr_us_reset_no_repair : us_selected_definition_needs_viewport_repair
  (us_mario._mario_reset_bodystate, Gfun (Internal (ibr_body VersionUS))) = false.
Proof. vm_compute; reflexivity. Qed.
Local Opaque normalize_global_definition_map.

Lemma ibr_us_reset_selected_member :
  In (us_mario._mario_reset_bodystate, Gfun (Internal (ibr_body VersionUS)))
...
Proof.
  ...
  - unfold repair_us_selected_global_definition. rewrite ibr_us_reset_no_repair. reflexivity.
  - apply every_selected_internal_body_is_preserved_verbatim. exact ibr_us_reset_selection.
Qed.

Lemma ibr_jp_reset_source :
  PTree.get us_mario._mario_reset_bodystate (prog_defmap (nlist_at 1 jp_cleaned_units)) =
    Some (Gfun (Internal (ibr_body VersionJP))).
Proof. vm_compute; reflexivity. Qed.

Theorem ibr_selected_reset_resolves : forall version, exists b,
  Genv.find_symbol (Clight.globalenv (selected_clight_target version))
    us_mario._mario_reset_bodystate = Some b /\
  Genv.find_funct_ptr (Clight.globalenv (selected_clight_target version)) b =
    Some (Internal (ibr_body version)).
Proof.
  intros [].
  - eapply program_definitions_resolve_internal_globalenv.
    ...
    + exact ibr_us_reset_selected_member.
