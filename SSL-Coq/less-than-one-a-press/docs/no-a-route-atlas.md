...
| 19 | Ink installation | [Negative quicksand depth plus stalled automatic dialog](#route-rank-19) | Very low; late helper writes are checked under separate live storage and audio/loading effects, but those conditions, other landing outcomes and the complete no-A history remain open |
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) requires timer four or later for a first negative landing calculation; with the checked ordinary duration bound preserved, only long-jump landing reaches that window. Leaving the ground instead resets the timer to zero. The [late-helper proof](notes/ink-negative-depth-history.md#the-late-helpers-are-no-longer-black-boxes) now checks the sound helpers' flag updates and the animation helper's object, buffer and loading-bookkeeping updates: with separate live storage and harmless audio/loading effects, none can revive that timer, so this final calculation cannot supply the first negative seed. Those live conditions and runtime effects still need proof. Checked input preparation rules out several substitutes for a new press, but the full controller and action history remains open. Ordinary Area-2 entry resets depth, and any hypothetical negative seed must still survive resets and move displayed height into real collision position; neither that setup nor Area-1 transport to the upper warp is established.

**What closes it.** For the leave-ground branch, establish that the live object, animation settings and animation buffer really occupy separate storage from Mario's movement record, then prove that the remaining audio request and animation transfer leave the landing timer alone, or show that a call is not reached. For other landing outcomes, follow the earlier duration bound through every intervening movement and helper call to the depth calculation. Connect ordinary controller samples and remembered input through the remaining input processing and action changes to the first long jump, while accounting for every other depth change and reset in the same run. A physical-press claim also needs the accepted starting input history and no demo-supplied A input. If this rules out every clean negative seed, stop before searching for dialog transport; otherwise the seed must survive and ultimately change real collision position. The [Area-2 hypothetical](notes/area2-negative-quicksand-star-hypothesis.md) and [negative-depth audit](notes/negative-quicksand-unreanchored-dialog.md) retain the limits; outside-model memory/code modification remains deferred.
