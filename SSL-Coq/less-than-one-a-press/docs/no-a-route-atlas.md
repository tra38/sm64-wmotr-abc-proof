...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), but its 960-unit displayed/collision height gap has no clean producer; the lower setup loses the top and the clean pillar recording arrives too late. The [backward proof](notes/ink-backward-search.md) shows that ordinary copies transfer existing heights and a completed ground step refreshes the display from movement. Crawling/sliding alignment can then lower movement while leaving the display behind, but needs an earlier mismatch with the remembered floor. The [floor proof](notes/ink-floor-history.md) distinguishes that floor from the new query: some blocked-step cases remain open. The [shared-history proof](notes/ink-shared-history-invariants.md#behavior-command-to-the-shared-action-prefix) now follows the actual behavior command into Mario's action update without changing memory, then joins the checked visibility/body reset under stated storage conditions. Reaching that command from fresh entry, live storage and later movement remain unproved. The [conditional search](notes/ink-conditional-100-coin-setup.md) grants valid coin/star placement and covers both milestone-triggering and ordinary starting totals. The milestone check cannot change depth or positions; a stalled popup amplifies an existing negative-depth effect rather than supplying the seed. Neither a useful negative seed nor a clean alignment mismatch is known.

**What closes it.** Establish the [shared and branch-specific invariants](notes/ink-shared-history-invariants.md) from fresh, normally initialized level-select entry and preserve them through every possible execution. Connect the live references and scheduling to the checked prefix, then classify input preparation, queries, actions and the remaining update. Either find a large movement/floor mismatch surviving a blocked crawling/sliding step or prove every reachable mismatch too small or reset, keeping the remembered and new floors separate. Also close the negative-depth seed or identify another allowed height change. The granted reward setup postpones coin-placement provenance, not ordinary collection, star timing, the save prompt or either milestone continuation; success with that allowance remains conditional. A useful gap must survive the intervening resets and copies, pillar activation, the still-present top, warp contact and both retry lookups without a new A press. The 960-unit gap belongs to the known setup; another top point needs its own checks. No castle-entry or new downstream-star continuation is required. Outside-model memory/code modification remains deferred.
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) narrows an ordinary first negative landing calculation to long-jump landing under the checked timing and helper conditions. The [local checks](notes/ink-negative-depth-history.md) exclude carrying the old timer after leaving the ground and creating a new A-pressed flag through crouch-slide's timer update. The [shared prefix](notes/ink-shared-history-invariants.md#behavior-command-to-the-shared-action-prefix) now connects the real behavior command, Mario callback and action entry in one memory, and preserves the existing depth through visibility/body reset under stated storage conditions. The preceding fresh-entry execution, storage and later action history remain unproved. The [reward allowance](notes/ink-conditional-100-coin-setup.md) grants valid placement and milestone-triggering or ordinary starting totals, not negative depth. The milestone check preserves depth; the stalled-popup mechanism raises the display only with an existing negative seed. Temporary negative values clamped away before use are not surviving seeds. Ordinary Area-2 entry resets depth, and displayed height still needs a collision-height bridge. The whole-history claim remains open.

**What closes it.** Prove the [shared-history claim](notes/negative-depth-shared-closure.md): starting at fresh, normally initialized level-select entry, the first useful negative depth must be preceded by a new physical A press. Trace the actual useful read back to its surviving producer in one run, covering resets, actions, live storage and outside effects. Retain the granted coin/star arrangement and both milestone outcomes while checking their ordinary contact, timing and continuation; do not assume either the seed or a dialog that skips the star dance. Reuse the landing bounds and controller checks only after connecting their actual readings and timing. The ordinary long-jump case must lead back to its first constructor and a new controller press, not merely held A or demo input. The current status is “whole-history connection missing,” not “route closed.” If the seed is excluded for every allowed case, no dialog-transport search is needed. Outside-model memory/code modification remains deferred.
...
**What is already known.** A granted ground pound can leave Mario 260 units above the descending elevator floor, clearing the checked wall-height cutoff of 231, but it stops sideways speed and supplies no B/Z escape. The ordinary falling-wall response points inward in the existing diagnostic. The [entry check](notes/rank10a-elevator-entry-checks.md) covers both normal elevator jolts: following the same base does not leave Mario far enough above it to create freefall. The checked nearby geometry supplies no hangable ceiling, and rollout, jump-kick, dive and the initial drop cannot request ground pound directly. The [shared contact proof](notes/rank10a-ground-pound-moving-geometry.md#height-alone-cannot-replace-departure-2026-09-10) now follows a complete touch test: with ordinary target readings and distance calculation, extra height cannot collect a target while Mario's collision position stays inside the cage. A clean entry and useful sideways departure remain missing; changes of support and differences between Mario's movement and collision positions remain separate questions.
