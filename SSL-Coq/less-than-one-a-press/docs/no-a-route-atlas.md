...
**What is already known.** No clean no-A seed is known. The [conditional stock argument](notes/conditional-stock-negative-seed.md) connects a useful negative endpoint to physical A under explicit writer, timer, action and controller conditions. Early quicksand-jump landing erases its negative intermediate before the next helper; common landing has no matching clamp. The [late-landing investigation](notes/late-common-landing-investigation.md) now includes proofs for the cancellation check, six caller handoffs, animation's actual loader, the stock sound request leaving Mario and the duration records unchanged, and initialization choosing only standing or water idle. These are local results. Connecting the sound implementation to the gameplay model, protecting the records throughout earlier play, and completing the first-A history remain open. Six of twelve one-A controls reach late landing; none of twelve no-A controls does. Those are finite observations. Imported depth faces the proved course-entry reset. The [supplied display setup](notes/ink-vertical-installation.md) works conditionally, but no clean useful floorless arrival is established.

**What closes it.** Prove that a useful negative from this producer requires late landing and that late landing requires an earlier physical A press. Protect the stock duration records throughout earlier execution, preserve the checked timer through the remaining calls, and connect every first long-jump request to controller history. The sound implementation and initialization store are now proved locally; their surrounding gameplay connections are not all complete. Immediate erasure before a useful read excludes its own producer, as proved for early quicksand-jump landing. A cross-course candidate must account for the reset or a real producer afterward. No A-using seed-size calculation or later Ink continuation is needed. Neither the no-A seed family nor the whole route is closed.
