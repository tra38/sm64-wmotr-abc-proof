...
| 19 | Ink installation | [Negative quicksand depth plus stalled automatic dialog](#route-rank-19) | Very low; leaving ground clears the old landing timer, but later animation/sound effects, other landing branches and the full no-A action history remain open |
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) proves that a first negative landing calculation needs timer four or later. The [actual duration check](notes/ink-negative-depth-history.md) bounds the timer through the ordinary cancellation return; if later operations preserve that bound, only long-jump landing reaches the negative window, giving `-0.5` at four or `-4` at five. A new continuous landing proof excludes simply retaining that old timer when Mario leaves the ground: the action change resets it to zero and the dust update preserves zero. A first negative final calculation then needs a timer change during the later animation or landing-sound call; none is known. Checked input preparation also excludes held A, B/Z and stale input as substitutes for a new press, but the complete physical-input and action history remains open. Ordinary Area-2 entry resets depth; a supplied negative seed can cover smaller displayed-height gaps but still needs a successful copy into collision position. Neither that setup nor Area-1 transport to the upper warp is established.

**What closes it.** For the checked leave-ground branch, finish the effects of the two later animation and landing-sound calls. For other landing branches, connect the bounded cancellation return through every intervening movement and helper call to the depth calculation. Also connect ordinary controller samples and remembered history through the remaining input updates and action changes to the first long jump, deriving the ordinary landing settings and input flags. Keep the same Mario record and classify every other reached depth change and outside-call effect. A physical-press claim needs the accepted starting history and the absence of demo-supplied A input. If no clean seed exists, stop before searching for dialog transport; otherwise it must survive resets and connect displayed height to a successful floor retry and real collision position. The [Area-2 hypothetical](notes/area2-negative-quicksand-star-hypothesis.md), [writable-table audit](notes/writable-action-table-mutation.md), and [negative-depth audit](notes/negative-quicksand-unreanchored-dialog.md) retain the limits. Outside-model memory/code modification remains deferred.
