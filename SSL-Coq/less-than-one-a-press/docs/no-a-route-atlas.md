...
**What is already known.** On the successful zero-A four-pillar run, a read-only audit followed all 2,462 frames from that copy through the remaining objects and into the next frame. Mario stayed the same player object, the two positions matched after every copy, and neither position was written before the next platform update. They also matched at every checked collision entry and return. Separately, a new US/JP execution proof shows that the particle child-copy tail cannot move Mario's State, collision or display records after allocation returns a valid child in a different slot. New allocator proofs also track the free-slot result, derive a conditional freshness test from active flags, and show that initialization cannot write separate MarioState storage. Mario's neighboring Object slot and the whole interval still need protection; see the [allocation boundary](notes/f02-allocation-boundary.md). See the [trace](notes/rank5-state-split-trace.md) and [child-copy proof](notes/f02-postcopy-child-frame.md).

**What closes it.** The next bounded step is to prove allocation's writes stay within the child slot, check list and graph maintenance and eviction, and establish the live ownership and returned-child connection through spawning. Then cover the other reached callbacks through the next collision check. A general disproof must extend the resulting invariant to the relevant clean histories. A counterexample must identify the first wrong copy recipient or later position change and carry that disagreement into collision. Out-of-bounds corruption and arbitrary code execution remain outside the current execution model.
...
**What is already known.** The completed zero-A four-pillar run has write-by-write coverage across all 2,462 frames. Its only 7,386 writes to Mario's collision coordinates are the ordinary three-coordinate copies, with correct readbacks and no intervening retarget. Separately, the actual US/JP particle child-copy tail now has a proof that it preserves Mario's collision and display records when the returned child occupies a different valid slot. This closes that copy tail. The newer [allocator results](notes/f02-allocation-boundary.md) protect separate MarioState storage, but do not yet protect Mario's collision and display records within the shared Object pool; allocation and other callbacks remain open. Abstract examples still show why ordering alone cannot exclude other histories, but none is a clean gameplay witness. See the [copy/interaction audit](notes/area1-ranks13-18-copy-interaction-audit.md) and [child-copy execution proof](notes/f02-postcopy-child-frame.md).
