...
**What is already known.** No clean no-A seed is known. The [conditional stock argument](notes/conditional-stock-negative-seed.md) connects a useful negative endpoint to physical A under explicit writer, timer, action and controller conditions. Early quicksand-jump landing is excluded at its proved immediate-clamp checkpoint; common landing has no matching clamp. The [late-landing investigation](notes/late-common-landing-investigation.md) preserves the stock duration through continuing cancellation and now carries the timer below four into six real landing-body calls. It follows animation into its actual loader and transfer, and excludes the other safe-literal direct action assignments as long-jump constructors. The duration records have restricted ordinary reads but are exported, so the private-table proof does not establish their lifetime. Earlier descriptor protection, remaining call effects and the full first-A history remain open. Six of twelve one-A controls reach late landing; none of twelve no-A controls does. These are finite observations. An imported seed faces the proved course-entry reset. The separately [supplied display setup](notes/ink-vertical-installation.md) works conditionally, but no clean useful floorless arrival is established.

**What closes it.** Prove that a useful negative from this producer requires late landing and that late landing requires an earlier physical A press. Immediate erasure before any useful read also excludes its own producer, as proved for early quicksand-jump landing. For common landing, protect the exported duration records through earlier execution, carry the timer through the remaining calls, and connect every first long-jump request to controller history. The six wrapper handoffs, actual animation-loader connection and safe direct stores narrow these obligations; they do not complete them. No A-using seed-size calculation or later Ink continuation is needed for this exclusion. A cross-course candidate must account for the reset or a real producer afterward. Neither the no-A seed family nor the whole route is closed.
