...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), but its 960-unit displayed/collision height gap has no clean producer; the lower setup loses the top and the clean pillar recording arrives too late. Ordinary copies transfer existing heights, and completed ground steps refresh the display from movement. Crawling/sliding alignment can later lower movement while leaving the display behind, but needs a mismatch with the remembered floor; [blocked-step cases remain open](notes/ink-floor-history.md). Normal starting conditions are now accepted, and the [shared proof](notes/ink-shared-history-invariants.md#accepted-initial-storage-and-constructed-preparation) constructs one initial action-call execution through eleven bookkeeping writes to the actual button-processing call: depth stays zero, the tracked heights stay equal, and old input is cleared. The surrounding game-update order and later actions remain unproved. The [conditional reward allowance](notes/ink-conditional-100-coin-setup.md) grants valid coin/star placement and both milestone-triggering and ordinary starting totals; the milestone check preserves depth, and a stalled popup amplifies an existing negative seed rather than creating it. Neither a useful negative seed nor a clean alignment mismatch is known.
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) narrows the ordinary first negative landing calculation to long-jump landing under the checked timing and helper conditions. Local checks exclude retaining the old timer after leaving the ground or creating a new A-pressed flag through crouch-slide's timer update. Normal initial storage is now accepted, and the [shared proof](notes/ink-shared-history-invariants.md#accepted-initial-storage-and-constructed-preparation) constructs eleven consecutive bookkeeping writes through the actual button-call entry, preserving zero depth and clearing old input without assuming those operations completed. Later input processing, actions and game-update ordering remain unproved. The [reward allowance](notes/ink-conditional-100-coin-setup.md) permits valid placement and milestone-triggering or ordinary starting totals, not a negative seed; the milestone check preserves depth. Temporary negatives clamped away before use are not useful seeds. Ordinary Area-2 entry resets depth, and raised display height still needs to affect collision height. The whole-history claim remains open.
