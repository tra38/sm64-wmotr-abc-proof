...
**What is already known.** The supplied mid-face setup really does bypass the elevator: the [recording](notes/ink-area2-arrival-video.md) shows Mario landing on an upper walkway and reaching one puzzle secret without A; a separate continuation records conditional Act-6 collection. Clean installation remains open. The known setup needs a displayed/collision height gap of at least 960 units (1,010 at the warp center); an earlier, lower setup finds the top but loses it before the warp. Ordinary creation, normal behavior and checked display offsets supply no known clean producer, and the clean four-pillar recording reaches the warp too late. The [backward proof](notes/ink-backward-search.md) now shows that, with the ordinary live object reference and separate storage, the real US and JP retry writes exactly the displayed height already present when the retry begins: its local setup and earlier X copy cannot manufacture extra height. A non-null first floor skips the retry. These facts narrow the earlier setup without excluding every controller history.

**What closes it.** Work backward from the display height at retry entry to its last change or reset, proving along the way that Mario's live object reference remains the expected one. The copy's own earlier writes are now accounted for under ordinary storage; the remaining search must cover both display changes and collision-position changes. Either prove every allowed earlier history leaves too little separation, or find one clean producer that preserves a useful gap until the lookup. Any surviving setup must combine pillar activation, the still-present spinning top, warp contact, a failed first lookup, a successful retry and retained support in one no-A run reaching the useful downstream platform. A different top point needs its own height and support-lifetime checks; 960 units describes the known mid-face setup, not every possible installation. The accepted entry is sufficient; no castle-entry connection or new downstream gameplay search is needed here. Outside-model memory/code modification remains deferred.
