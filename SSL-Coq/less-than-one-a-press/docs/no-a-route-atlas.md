...
| 2 | Ink installation | [Timer-131 non-null Graphics retry](#route-rank-2) | Very low for a clean producer; recorded conditional elevator bypass |
...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), reaching an upper walkway; clean installation remains open. This setup needs at least a 960-unit displayed/collision height gap; a lower setup loses the top, and the clean pillar recording arrives too late. Checked ordinary behavior and display offsets provide no producer. Under ordinary separate storage and matching Mario references, the [US/JP backward proof](notes/ink-backward-search.md) shows that the retry and collision copies transfer existing heights, while the checked floor resets replace old display height with the sampled floor height. The entire quicksand sink is now checked too: with separately stored drawing data and finite input/output values, nonnegative depth cannot raise the displayed height, even through rounding, and the call does not lower movement or collision height either. An upward change under those conditions requires negative depth; no clean seed is established. The sampled floor is not proved low, and the history between these checkpoints, skipped resets, moving-ground branch and later changes remain open.

**What closes it.** Trace the required height difference back through the action before sinking and through every later change to the retry, in one run with Mario's correct live identity. Establish the sink's ordinary storage, finite values and nonnegative depth throughout that history; otherwise identify the exact defined exception. A checked reset passes the height question to its actual floor, while floor alignment, skipped resets, moving-ground movement and the platform/dialog case still need their complete timing checked. Either prove every allowed history leaves too little separation or find a clean producer whose useful gap survives. A survivor must combine pillar activation, the still-present top, warp contact, a failed first lookup, successful retry and retained support without a new A press. A different top point needs its own height and lifetime checks; 960 units describes the known setup, not every installation. The accepted start and useful downstream platform suffice; no castle-entry connection or new star-gameplay search is needed. Outside-model memory/code modification remains deferred.
