...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), but its 960-unit displayed/collision height gap has no clean producer; the lower setup loses the top and the clean pillar recording arrives too late. The [backward proof](notes/ink-backward-search.md) shows that ordinary copies transfer existing heights and a completed ground step refreshes the display from movement. Crawling/sliding alignment can then lower movement while leaving the display behind, but needs an earlier mismatch with the remembered floor. The [floor proof](notes/ink-floor-history.md) distinguishes that floor from the new query: some blocked-step cases remain open. The [shared-history checks](notes/ink-shared-history-invariants.md) preserve the tracked readings through the initial visibility/body reset under stated storage conditions; live storage, scheduling and later movement remain unproved. A separate [conditional search](notes/ink-conditional-100-coin-setup.md) now grants valid coin/100-coin-star placement and retains both milestone outcomes. The complete milestone-check helper cannot change depth or the tracked positions, and a stalled popup amplifies an existing negative-depth effect rather than supplying the seed. Neither a useful negative seed nor a clean alignment mismatch is known.
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) narrows an ordinary first negative landing calculation to long-jump landing under the checked timing and helper conditions. The [local checks](notes/ink-negative-depth-history.md) exclude carrying the old timer after leaving the ground and creating a new A-pressed flag through crouch-slide's timer update. The [shared prefix](notes/ink-shared-history-invariants.md) preserves zero depth through visibility/body reset when the live storage is separate, but does not prove that storage, scheduling or later action history. The [new reward allowance](notes/ink-conditional-100-coin-setup.md) grants valid coin/star placement and covers both milestone outcomes, not negative depth. The complete milestone-check helper preserves depth; the known stalled-popup mechanism accumulates displayed height only with an already-negative seed. Temporary negative values clamped away before use are not surviving seeds. Ordinary Area-2 entry resets depth, and displayed height still needs a real collision-height bridge. The whole-history claim remains open.
