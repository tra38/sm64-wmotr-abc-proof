...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), reaching an upper walkway; clean installation remains open. This setup needs at least a 960-unit displayed/collision height gap; a lower setup loses the top, and the clean pillar recording arrives too late. Checked ordinary behavior and display offsets provide no producer. Under ordinary separate storage, the [US/JP backward proof](notes/ink-backward-search.md) shows that the retry copies an existing display height, while the checked floor-reset paths replace old display height with the newly sampled floor height. The ordinary collision copy now has an exact check too: with matching Mario references, it selects Mario's movement record and copies its incoming height without changing the incoming display height; its four preceding writes preserve both readings. It can transfer an existing movement/display difference into a collision/display gap, but cannot invent the missing height. The sampled floor is not proved low, and the history between these checkpoints, skipped resets, moving-ground branch and later changes remain open.

**What closes it.** Trace the movement/display difference required at collision-copy entry back to the last display change, then connect the remaining copy statements to the retry in one run with Mario's correct live identity. A checked reset passes the height question to its actual selected floor; its remaining statements and the action tail still need checking, alongside movement during skipped resets and the already-open platform/dialog case. Either prove every allowed history leaves too little separation or find a clean producer whose useful gap survives to the lookup. A survivor must combine pillar activation, the still-present spinning top, warp contact, a failed first lookup, successful retry and retained support without a new A press. A different top point needs its own height and lifetime checks; 960 units describes the known setup, not every installation. The accepted start and useful downstream platform suffice here; no castle-entry connection or new star-gameplay search is needed. Outside-model memory/code modification remains deferred.
