...
| 19 | Ink installation | [Negative quicksand depth plus stalled automatic dialog](#route-rank-19) | Very low; actual late-landing and A-guard checks strengthen the seed exclusion, but the whole no-A history is not yet proved |
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) now proves that the actual rounded landing write needs timer four or later to turn nonnegative depth negative; under stock timing, only long-jump landing reaches that window. The real crouch-slide launch guard skips when its A-pressed bit is clear, but connecting these checks to every live controller history remains open. Crossing onto quicksand can give `-0.5` at timer four or `-4` at timer five, so both need covering. Ordinary Area-2 entry resets depth to zero. Its Act-6 standing gap is 11 collision units over quicksand; Act 3's is 75 over ordinary floor. The conditional `-2.65` seed supplies enough display height after five or 29 sinks respectively, but only a later copy into collision position could affect collection. Neither the required failed floor lookup nor the Area-1 transport to the upper warp is established.

**What closes it.** From the accepted zero-depth start, connect the late landing write to its earlier timer limit, follow long-jump landing back to the first long-jump entry, and derive the A-pressed bit from the actual controller history. Keep the same Mario record and ordinary landing settings throughout, classify every other reached depth/action change, and account for each reached outside call. This would prove the seed needs A, or expose one concrete allowed alternative. If no clean seed exists, stop before searching for dialog transport; if one exists, it must survive resets and connect the accumulated display height to a successful floor retry and real collision position. The [Area-2 hypothetical](notes/area2-negative-quicksand-star-hypothesis.md), [writable-table audit](notes/writable-action-table-mutation.md), and [negative-depth audit](notes/negative-quicksand-unreanchored-dialog.md) retain the detailed limits. Memory/code modification outside the current model remains deferred, not an unfinished gameplay producer.
