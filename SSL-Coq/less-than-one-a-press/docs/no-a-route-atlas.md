...
**What is already known.** No clean no-A seed is known. The [conditional stock argument](notes/conditional-stock-negative-seed.md) proves that a useful negative endpoint needs physical A under its explicit writer, timer, action and controller conditions. Eight direct writer names are checked. Early quicksand-jump landing is now excluded as a source of a surviving negative: its actual US/JP subtraction is clamped before the next helper, under the stated timer and finite-arithmetic conditions. Common landing has no matching clamp after its last depth write, so that exclusion does not transfer to it. The [late-landing investigation](notes/late-common-landing-investigation.md) connects all nine direct guards to the real cancellation and timer read. The complete continuing cancellation now also preserves the separate descriptor: a stock duration at entry stays four or six, so only long-jump landing can return timer 4 or 5 there. The complete central action setter cannot convert a different requested action into either long-jump action. Earlier descriptor lifetime, later timer effects and the first-request/controller history remain open. Six of twelve one-A controls reach late landing; none of twelve no-A controls does, and none records negative depth at an update boundary. These remain finite observations. The [wider game review](notes/negative-depth-course-transfer.md) finds the same landing mechanism in other quicksand courses, but an imported value cannot pass unchanged through the proved initialization reset. Separately, the [supplied display setup](notes/ink-vertical-installation.md) works conditionally; the moving dialog trials still find a floor and refresh the display. No clean useful floorless arrival has been established.

**What closes it.** Prove that every useful negative from this producer requires late landing, and that reaching late landing requires an earlier physical A press. An immediate clamp before any useful read also closes its own producer; the early quicksand-jump pair is proved this way. For common landing, preserve descriptor values through the earlier gameplay to guard entry, carry the returned timer bound through the following calls, and connect every first long-jump request and other action write to the physical controller history. The new cancellation and setter proofs discharge parts of those connections, not the whole history. No A-using seed-size calculation or later Ink continuation is needed once the necessary-condition chain is complete. A cross-course candidate must account for the reached reset or a real producer afterward. For the separate transfer test, negative depth and a valid coin opportunity remain agreed grants, but useful floor loss, contact and timing still need a gameplay explanation. Neither the no-A seed family nor the whole route is closed.
