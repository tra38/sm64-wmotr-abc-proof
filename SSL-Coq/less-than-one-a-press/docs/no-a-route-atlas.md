...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) narrows an ordinary first negative landing calculation to long-jump landing, provided the checked timing and helper conditions hold. The [helper and controller checks](notes/ink-negative-depth-history.md) rule out several local shortcuts, including carrying the old timer after leaving the ground and creating a new A-pressed flag through crouch-slide's timer update. A [shared preparation proof](notes/negative-depth-shared-closure.md#shared-preparation-prefix-now-constructed) now connects the first five stages of one completed action pass and locates which stage first changes the observed depth; it does not yet establish that pass from the agreed start or explain every change inside its helpers. Earlier movement, the complete action and controller history, and the identity of Mario's live record remain unproved, so this route is not closed. Ordinary Area-2 entry resets depth, and a surviving seed must still turn displayed height into real collision height before helping collect a star.
