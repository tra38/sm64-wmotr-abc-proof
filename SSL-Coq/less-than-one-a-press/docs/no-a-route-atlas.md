...
**What closes it.** Check the earlier list and graph maintenance and the full-pool eviction branch, establish live ownership at each allocation, and carry the returned slot through the remaining spawning wrappers. The initializer's slot bounds and the complete nonempty allocator's returned-slot connection are now proved, but they do not establish those earlier or later effects. Then cover the other reached callbacks through the next collision check. A general disproof must extend the resulting invariant to the relevant clean histories. A counterexample must identify the first wrong copy recipient or later position change and carry that disagreement into collision. Out-of-bounds corruption and arbitrary code execution remain outside the current execution model.
