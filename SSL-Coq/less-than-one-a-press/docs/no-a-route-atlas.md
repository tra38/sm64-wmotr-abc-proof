...
**What is already known.** Both the supplied midpoint and the [vertical-only setup](notes/ink-vertical-installation.md) bypass the elevator in JP. The latter uses actual and collision position `(-2200,768,-1024)` and display `(-2200,1938.8648681640625,-1024)`. The real retry selects the top and retains it through explosion, warp and the first useful Area-2 movement; Coq separately checks the recorded floor selection. That successful setup was supplied with zero depth. The [three-second continuation search](../instrumentation/jp-dialog-support-search/README.md) instead lets the game raise the display from a supplied negative-depth dialog checkpoint. A moving-top trial changes actual X/Z while preserving that display, but its first query finds a floor and ordinary movement immediately refreshes the display. The complete generated US/JP platform-movement phase now provably preserves both display and collision coordinates in the normal Object pool, including every helper call and any rotation. Moving actual Mario therefore cannot itself supply a new low collision position. Static support keeps its floor, while an ownerless floor clears a remembered platform. The useful gameplay producer remains missing.

**What closes it.** Find a gameplay predecessor whose movement or support loss also makes the first query fail while preserving the useful display, warp contact and top timing. A platform-based producer needs the low collision position before platform movement, with a usable platform and raised display already present, or a specific surrounding writer that supplies contact before it is checked. The tested moving-top departure is too high and still finds a floor. Starting at the warp centre still needs the westward departure, which a floor-height snap does not supply. Other support poses, action changes and a retained floor-alignment mismatch remain open, as does recovering the supplied dialog checkpoint from a real reward collection. Negative depth may be granted while testing transfer. The finite search and local execution proofs do not exclude all controller histories; clean reachability and the whole route remain open. Startup reconstruction and another downstream-star search are not prerequisites.
...
**What is already known.** A stalled milestone dialog can amplify negative depth, but no clean no-A seed is known. The [reward allowance](notes/ink-conditional-100-coin-setup.md) retains valid coin/star contact and both milestone outcomes. The [supplied vertical display](notes/ink-vertical-installation.md) gives a useful JP installation from idle with zero depth. The conditional dialog tests start with no display gap and let actual sinking build it. They supply the dialog checkpoint as an extra diagnostic assumption, not as a consequence of the reward allowance. On moving-top support, actual Mario can move before the raised display is refreshed, but the tested first lookup succeeds and the gap is then reset. The complete US/JP platform phase preserves the normal Object-pool collision position as well as display, so that motion cannot create low warp contact by itself. During the dialog itself, warp interactions are skipped. A floorless actual position with useful display, contact and timing still has no gameplay producer.
