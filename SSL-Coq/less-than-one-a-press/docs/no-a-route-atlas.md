...
| 19 | Ink installation | [Negative quicksand depth plus stalled automatic dialog](#route-rank-19) | Very low; input preparation and the landing timer gate are checked, but the complete no-A action history and later timer effects remain open |
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) proves that a landing can first make depth negative only at timer four or later. The [actual duration check](notes/ink-negative-depth-history.md) now bounds the stored timer through the cancellation routine's normal return, with ordinary landing settings and clear jump/off-floor flags; wrapping the timer cannot evade that check. If later helpers preserve this bound, only long-jump landing reaches the negative window, giving `-0.5` at four or `-4` at five. The checked input preparation excludes held A, B/Z and stale input as substitutes for a new press, and the consecutive controller writes remember the same sample without changing its detected edge. The full action history and later timer effects remain open. Ordinary Area-2 entry resets depth; a supplied negative seed can cover the smaller star gaps in displayed height, but still needs a successful copy into collision position. Neither that setup nor Area-1 transport to the upper warp is established.

**What closes it.** Connect ordinary controller samples and remembered history through the remaining input updates and action changes to the first long jump. Derive the ordinary settings and input flags at the landing check, then follow every movement, animation and sound call from its now-bounded return to the depth write, proving each preserves the timer or makes a harmless reset. Keep the same Mario record, classify every other reached depth change, and give each reached outside call its actual effect. A physical-press claim also needs the accepted starting history and the absence of demo-supplied A input. If no clean seed exists, stop before searching for dialog transport; otherwise it must survive resets and connect displayed height to a successful floor retry and real collision position. The [Area-2 hypothetical](notes/area2-negative-quicksand-star-hypothesis.md), [writable-table audit](notes/writable-action-table-mutation.md), and [negative-depth audit](notes/negative-quicksand-unreanchored-dialog.md) retain the limits. Outside-model memory/code modification remains deferred.
