...
**What is already known.** Both the supplied midpoint and the [vertical-only setup](notes/ink-vertical-installation.md) now bypass the elevator in JP. The latter uses actual and collision position `(-2200,768,-1024)` and display `(-2200,1938.8648681640625,-1024)`. Starting idle, the real retry selects the live top, retains it through the explosion and warp, and applies its leftover movement in Area 2 without A input. Coq separately checks the recorded floor lists against the generated code and existing mesh results. The setup was supplied with zero depth; it is not a dialog-produced state. The low point is 152 units west of the warp centre. An accepted warp interaction stops ordinary movement, and ground movement refreshes the display; the useful retry must happen earlier. The backward platform check now proves that an ownerless floor clears both remembered-platform references and that the next null-platform dispatcher cannot move Mario. A departure after that clearing needs another change to the remembered platform. No clean producer of the required gap is known.

**What closes it.** Work backward from the successful setup to a gameplay producer: a raised display, an eligible warp contact, and actual position or support changing before the next first floor lookup. Starting at the warp centre still needs the westward departure; a floor-height snap does not supply it. Waiting on unchanged support does not trigger the retry, and a failed ground step does not commit a floorless endpoint. A platform departure needs an earlier owned-floor result or a specific replacement of the cleared pointer, with the intervening calls checked. Negative depth and a retained floor-alignment mismatch remain separate candidates, with the produced depth, display coordinates, wall corrections and top timing all accounted for. The conditional test is complete; clean reachability and the whole route remain open. Closing the route requires covering every surviving producer, not merely failing to find one. Startup reconstruction and another downstream-star search are not prerequisites.
...
**What is already known.** A stalled milestone dialog can amplify an existing negative-depth display gap, but no clean no-A negative seed is known. The [reward allowance](notes/ink-conditional-100-coin-setup.md) retains valid coin/star contact and both milestone outcomes. The [supplied vertical display](notes/ink-vertical-installation.md) now gives a useful JP installation, tested from idle with zero depth. During the actual automatic dialog, warp interactions are skipped; its final change to idle does not run ordinary movement in that same update. This leaves a possible next-update transfer window. However, an ownerless final floor clears the remembered platform, and the complete next null-platform dispatcher changes no memory. Neither an earlier usable moving support nor a floorless actual position coexisting with the raised display has been derived from gameplay.

**What closes it.** Recover the post-dialog transfer before returning to the seed question. Track the last display refresh, the stalled dialog and its release, then find a real pre-action movement or loss of support that triggers the useful retry. A checked half-unit sinking example can reach a suitable display height from supported Y=1280; that particular example still needs a 512-unit actual drop without erasing the display. This is not a requirement for every producer: a start at the warp centre instead needs a 152-unit sideways departure. The resulting depth and remaining state must support the continuation. Negative depth may be granted for this investigation; later prove or refute its no-A creation from the accepted normal SSL start. The active-dialog interaction case is closed; the post-dialog producer and whole route are not.
