...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), reaching an upper walkway; clean installation remains open. It needs a displayed/collision height gap of at least 960 units, while an earlier lower setup loses the top before the warp. Checked ordinary behavior and display offsets provide no producer, and the clean pillar recording arrives too late. Under ordinary separate storage, the [backward proof](notes/ink-backward-search.md) shows that the real US and JP retry copies an already-existing display height; its own earlier writes cannot add height, and a non-null first floor skips it. The new floor-reset proof shows that the stop routine and the stationary routine's no-moving-ground branch write the newly sampled floor height to the display, using the object reference saved at that call's entry. A downward snap cannot retain the old high display through that assignment, but the sampled floor is not proved low. Later changes, skipped resets and the moving-ground alternative remain open; these checks do not exclude every controller history.

**What closes it.** Connect the last display change to the retry in one run, preserving Mario's correct live identity and checking collision changes alongside display changes. A checked floor reset passes the height question back to its actual selected floor; also follow later collision-position copies and movement during actions that skip the reset, including the already-open platform/dialog case. Either prove every allowed history leaves too little separation or find a clean producer that keeps a useful gap until the lookup. A survivor must combine pillar activation, the still-present spinning top, warp contact, a failed first lookup, successful retry and retained support without a new A press. A different top point needs its own height and lifetime checks; 960 units describes the known setup, not every installation. The accepted start and useful downstream platform are sufficient here; no castle-entry connection or new star-gameplay search is needed. Outside-model memory/code modification remains deferred.
