...
> Status snapshot: 2026-09-08.  Rankings are intentionally revisable as linked
...
| 2 | Ink installation | [Timer-131 non-null Graphics retry](#route-rank-2) | Very low; recorded conditional bypass, but floor alignment still needs a clean movement/floor mismatch |
...
| 19 | Ink installation | [Negative quicksand depth plus stalled automatic dialog](#route-rank-19) | Very low; held A and stale input cannot supply the needed new-press flag at the checked input update, but the complete controller/action/timer history remains open |
...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), but its 960-unit displayed/collision height gap has no clean producer; the lower setup loses the top and the clean pillar recording arrives too late. Under ordinary separate storage and matching Mario references, the [US/JP backward proof](notes/ink-backward-search.md) shows that the retry and collision copies transfer existing heights, while checked floor resets replace old display height. A completed ground step now reaches a checked refresh that copies current movement height, not an older display. Crawling/sliding alignment can then lower movement while retaining that display at its first snap, but this needs an earlier movement/floor mismatch or an intervening change; a blocked step can retain such a mismatch, so this case is not closed. The complete quicksand sink preserves movement/collision height and cannot raise a finite display with nonnegative depth. Its negative seed, the actual floor history, intervening helpers and survival to the retry remain unproved.

**What closes it.** Follow the blocked crawling/sliding case backward to the earlier floor query and movement: either find a clean, large movement/floor mismatch or prove every reachable one too small. Keep accepted grounded steps and steps that leave the ground separate, and check all helpers between refresh, alignment, collision copying and the retry in one run with Mario's correct live identity. Also close the negative-depth seed or identify another allowed height change; skipped resets and platform/dialog movement still need their own accounting. Any useful gap must survive pillar activation, the still-present top, warp contact, the first failed floor lookup and the successful retry without a new A press. The 960-unit bound describes the known setup; another top point needs its own height and lifetime checks. The accepted start and useful downstream platform suffice, with no castle-entry or new star-gameplay search. Outside-model memory/code modification remains deferred.
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) proves that the rounded landing write needs timer four or later; under stock timing, only long-jump landing reaches that window. A new US/JP execution proof follows input clearing through the complete button update: held A, B/Z, old input flags and button-age counters cannot create the required new-press flag there when the controller reports no A edge. The later input/action history and landing-timer connection remain open; built-in demo input and an unmatched starting button history are not established clean SSL alternatives. Crossing onto quicksand can give `-0.5` at timer four or `-4` at timer five. Ordinary Area-2 entry resets depth; its Act-6 gap is 11 collision units over quicksand, while Act 3's is 75 over ordinary floor. A conditional `-2.65` seed supplies enough display height after five or 29 sinks, but still needs a successful copy into collision position. Neither that failed-floor/retry setup nor Area-1 transport to the upper warp is established.

**What closes it.** Connect ordinary controller samples and their remembered history to the now-checked input preparation, then follow the remaining input updates and action changes to the first long jump. Link the late landing write to its earlier timer limit, keeping the same Mario record and ordinary landing settings; classify every other reached depth change and give each reached outside call its actual effect. This would prove the seed needs a new sampled A press, or expose one concrete allowed alternative; a claim about a physical press also needs the accepted starting history and the absence of demo-supplied A input. If no clean seed exists, stop before searching for dialog transport. Otherwise it must survive resets and connect displayed height to a successful floor retry and real collision position. The [Area-2 hypothetical](notes/area2-negative-quicksand-star-hypothesis.md), [writable-table audit](notes/writable-action-table-mutation.md), and [negative-depth audit](notes/negative-quicksand-unreanchored-dialog.md) retain the limits. Outside-model memory/code modification remains deferred.
