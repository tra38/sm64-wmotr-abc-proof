...
**What is already known.** The supplied mid-face setup really does bypass the elevator: the [new recording](notes/ink-area2-arrival-video.md) shows Mario reaching Area 2 outside the shaft, landing on an upper walkway and reaching one puzzle secret without A; a separate existing continuation records conditional Act-6 collection. Clean installation and an Act-3 continuation remain open. The known setup needs a displayed/collision height gap of at least 960 units (1,010 at the warp center); an earlier, lower setup finds the top but loses it before the warp. Ordinary creation, normal behavior and checked display offsets supply no known clean producer. The accepted JP entry and recorded later updates keep Mario's identity and relevant values safe; a clean four-pillar run reaches timer 131 but reaches the warp too late. The [backward execution proof](notes/ink-backward-search.md) shows that a non-null first floor skips the retry and that the retry copies height rather than adding it. These facts constrain the earlier setup, not every controller history.

**What closes it.** Work backward from the height actually read by the copy to Mario's display record and its last change or reset, accounting for the earlier writes and the live Mario-object reference. Either prove every allowed earlier history leaves too little separation, including changes to collision position, or find one clean producer that preserves a useful gap until the lookup. Any surviving setup must combine the pillar activation, still-present spinning top, warp contact, failed first lookup, successful retry and retained support in one no-A run, then finish each star's continuation. A different top point needs its own height and support-lifetime checks; the 960-unit bound describes the known mid-face setup, not every possible installation. The accepted entry remains sufficient for this search; no castle-entry connection is required. Outside-model memory/code modification remains deferred, not a producer to develop here.
