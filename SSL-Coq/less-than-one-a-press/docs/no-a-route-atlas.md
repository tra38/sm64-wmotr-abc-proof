...
**What is already known.** The [proof and timing check](notes/rank10a-ground-pound-moving-geometry.md) confirm a conditional height window: a normal ground pound starting at the descending elevator floor could leave Mario 260 units above it after the fifteen-update pause, exceeding the checked wall-height cutoff of 231. The ten planned lifts total 110 units; startup stops sideways speed and has no B/Z escape, while ordinary wall and floor checks still run. An offline test of every facing angle against all four inner sides now shows that an ordinary falling-wall hit launches Mario inward, not through that same wall; corners, later collisions and floor/ceiling-triggered responses remain separate. Freefall and ceiling hanging can request ground pound, but the tested rollout, jump-kick, dive and initial upper-entry drop cannot do so directly. Following the elevator's ten-unit descent does not itself create freefall. No clean eligible entry, useful departure or star collection is established.

**What closes it.** Find a controller-reachable entry and a specific sideways help or departure during the brief height window, then follow the actual support movement, wall and floor checks, headroom tests, interruption or falling-wall response, landing and star collection in one run. Otherwise prove that every eligible entry is unavailable or every such continuation returns harmlessly. Another moving support remains a separate case, and an interrupted or restarted ground pound needs its own full accounting; neither is ruled out by the elevator arithmetic. Any time-stop version must use the game's actual update rules.
