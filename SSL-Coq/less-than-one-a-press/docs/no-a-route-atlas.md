...
**What is already known.** The supplied mid-face setup retains the top into Area 2 and reaches one puzzle secret, but needs a displayed/collision height gap of at least 960 units (1,010 at the warp center); it does not yet collect both stars. An earlier, lower setup finds the top but loses it before the warp. Ordinary creation, Mario's normal behavior, and the checked stock display offsets supply no known clean producer. The accepted JP entry and recorded later updates keep Mario's identity and relevant values safe; a clean four-pillar run reaches timer 131 but reaches the warp too late. The new [backward execution proof](notes/ink-backward-search.md) shows that a non-null floor at the actual first test skips the retry, and that the retry's copy reads its Y value before assigning it—it does not add height. These facts narrow the necessary earlier setup; they do not cover every controller history.
