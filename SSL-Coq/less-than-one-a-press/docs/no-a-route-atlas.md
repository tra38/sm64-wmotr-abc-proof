...
**What closes it.** The existing conditional theorem is a possible stopping point if its listed scope is accepted: keep the timer-at-write, first-entry/controller and, for the whole seed family, writer-coverage conditions visible. This review does not accept them on the user's behalf. Removing those conditions requires protecting the stock duration records through earlier execution, preserving the bound through the remaining calls, and completing the first-long-jump/input history. The sound binding is part of that justification, not a new gameplay objective. Immediate erasure before a useful read excludes its own producer, as proved for early quicksand-jump landing. A cross-course candidate must account for the reset or a real producer afterward. No A-using seed-size calculation or later Ink continuation is needed. The unrestricted no-A seed family and the whole route remain open.
