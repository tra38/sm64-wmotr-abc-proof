...
**What is already known.** The supplied setup [bypasses the elevator](notes/ink-area2-arrival-video.md), but its 960-unit displayed/collision height gap has no clean producer; the lower setup loses the top and the clean pillar recording arrives too late. The [backward proof](notes/ink-backward-search.md) shows that ordinary copies transfer existing heights and a completed ground step refreshes the display from movement. Crawling/sliding alignment can then lower movement while leaving the display behind, but requires an earlier mismatch with the remembered floor. The [floor proof](notes/ink-floor-history.md) keeps that remembered floor separate from the newly queried one: the checked high branch either blocks at a ceiling or leaves the ground and skips alignment, while a different new floor or an earlier missing-floor block remains open. The [shared-history extension](notes/ink-shared-history-invariants.md) checks that the initial visibility and body-reset steps preserve the tracked height and depth readings under the stated storage conditions. Those storage facts, real scheduling and later movement still need connection. Sinking cannot raise a finite display from nonnegative depth; neither a useful negative seed nor a clean alignment mismatch is known.

**What closes it.** Establish the [shared and branch-specific invariants](notes/ink-shared-history-invariants.md) from the accepted start and preserve them through every possible execution. Connect the live references and scheduling to the checked common prefix, then classify input preparation, queries, actions and the remaining update. Either find a large movement/floor mismatch that survives a blocked crawling/sliding step or prove every reachable mismatch too small or reset, keeping the remembered and new floors separate. Also close the negative-depth seed or identify another allowed height change. A useful gap must survive display refresh, alignment, sinking, collision copying, pillar activation, the still-present top, warp contact and both retry lookups without a new A press. The 960-unit gap belongs to the known setup; another top point needs its own checks. The accepted start and useful downstream platform suffice, without castle-entry or new star-gameplay work. Outside-model memory/code modification remains deferred.
...
**What is already known.** No clean no-A negative seed is known. The [backward search](notes/negative-quicksand-unreanchored-dialog.md#walking-backward-from-the-first-negative-seed) narrows an ordinary first negative landing calculation to long-jump landing under the checked timing and helper conditions. The [local checks](notes/ink-negative-depth-history.md) exclude carrying the old timer after leaving the ground and creating a new A-pressed flag through crouch-slide's timer update. The [shared action-prefix proof](notes/ink-shared-history-invariants.md) now connects the initial visibility update and complete body reset, preserving zero depth and the tracked height readings when the live body storage is separate. It does not establish that storage fact at the accepted start, schedule the call, or classify the following input preparation and action loop. Temporary negative values clamped away before use are not surviving seeds. The complete action/controller history remains unproved, so this route is not closed. Ordinary Area-2 entry resets depth, and a surviving seed must still turn displayed height into real collision height to help collect a star.
