...
finds no path there for any of the nine starts, while its graph allowing a
generous hypothetical push finds a path for every start. Neither result is
a complete gameplay verdict: the first misses some airborne behavior, and
the second does not arrange another Goomba or a real collision. The western,
...
native-command dispatcher are now connected across any finite number of
checks under the stated preservation, raw-Mario and named-library contract.
That conditional fresh-triplet claim is proved. A separate explicit local
binding connects the square-root implementation to the distance helper;
the older unspecified-call semantics is not silently equated with it.
Deriving the sequence's conditions through all ordinary gameplay remains
open, including Mario's raw position at the distance reads.
