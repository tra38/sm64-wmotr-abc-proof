...
| The constructor's input comes from a new physical press | Controller edge, remembered-input, complete button-helper results and `InkInitialControllerGuard` | The accepted initial controller word now survives the actual preparation writes and reaches the real A-pressed test, which is not taken. The complete-button no-A result applies at that same call; its remaining writes also preserve depth/action/timer. Later samples, input processing, action constructors and the full physical-sample history remain to connect. |
