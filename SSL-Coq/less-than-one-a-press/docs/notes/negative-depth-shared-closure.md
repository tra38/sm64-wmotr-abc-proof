...
| The surviving producer is one of the checked depth cases | `NegativeDepthDefinedProducerClosure`; `InkInputSharedConstruction`; `InkInputGeometryHistory`; the earlier action and landing results | Eleven preparation writes are constructed. Actual consecutive button and joystick calls now extend that same history through their returns to the first wall-query statement, preserving zero depth, the initial action and timer. The joystick and both angle helpers have proved effects; completion of the two calls is still execution evidence. Geometry, post-boundary scheduler coverage and the later surviving-producer classification remain missing. |
...
| The constructor's input comes from a new physical press | Controller edge, remembered-input, complete button-helper results, `InkInitialControllerGuard` and `InkJoystickFrame` | The accepted initial sample reaches the real A-pressed test without being replaced. The complete-button result and the now-classified joystick call keep that input clear in their consecutive execution up to geometry. This covers all successfully executed joystick branches, not just a chosen direction. Later physical samples, geometry, action constructors and intervening world updates remain to connect. |
