...
The [nine-wrapper and cancellation result](late-common-landing-investigation.md) belongs in Section 01 as a generated US/JP execution result. Each wrapper exits in its prechecks or guard, or continues after the real cancellation returned zero. The actual stock arguments, increment and duration read are connected. The continuing call now also preserves the separate descriptor block: a stock duration at guard entry stays four or six through return, and only long-jump landing can return timer 4 or 5 there. All four actual action initializers and the complete central setter also exclude a long-jump conversion from a different requested action. Expand the existing card with these scoped exclusions; keep 35 summaries and do not promote the whole producer. Earlier descriptor lifetime, later timer effects and the first-request/controller history remain open. The paired Wafel controls remain finite evidence.
