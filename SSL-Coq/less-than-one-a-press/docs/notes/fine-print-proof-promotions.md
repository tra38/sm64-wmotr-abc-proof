...
The [nine-wrapper and cancellation result](late-common-landing-investigation.md) belongs in Section 01 as a generated US/JP execution result. Each wrapper exits in its prechecks or guard, or continues after the real cancellation callee returned zero. The real stock arguments now connect that zero return to the actual duration read, incremented timer and unchanged post-gate memory. The returned timer is strictly below the duration it read; neither value is supplied as a premise. Expand the existing caller card rather than counting another whole route as closed. Preserving the stock four-or-six descriptor values, the later timer and the first-constructor/input history remains open. The paired Wafel controls stay finite evidence and do not receive an Already proved badge.
